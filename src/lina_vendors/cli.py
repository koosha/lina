"""`lina-vendors` CLI entry point — Click-based, JSON stdout, structured stderr."""
...
from lina_vendors.indices.runner import IndexRunner, list_pending
...
def seed_cmd(ctx: click.Context, reset: bool, named_only: bool, bulk_only: bool) -> None:
    ...
    if named_only:
        runner = IndexRunner(client=client, mappings_dir=_mappings_dir())
        if reset:
            if client.indices.exists(index="vendor_lawyer_profiles_v1"):
                client.indices.delete(index="vendor_lawyer_profiles_v1")
            if client.indices.exists(index="lina_vendors_index_state"):
                client.indices.delete(index="lina_vendors_index_state")
        runner.apply_pending()
        actions = [
            {"_index": "vendor_lawyer_profiles_v1", "_id": d["timekeeper_id"], "_source": d}
            for d in named_timekeeper_docs()
        ]
        helpers.bulk(client, actions, refresh="wait_for")
        client.indices.refresh(index="vendor_lawyer_profiles_v1")
        result = {"named": len(actions), "generated": 0, "total": len(actions)}
    elif bulk_only:
        runner = IndexRunner(client=client, mappings_dir=_mappings_dir())
        runner.apply_pending()
        gen = generate_timekeepers(count=200, seed=42)
        actions = [
            {"_index": "vendor_lawyer_profiles_v1", "_id": d["timekeeper_id"], "_source": d}
            for d in gen
        ]
...
def list_templates_cmd() -> None:
    from lina_vendors.templates import all_templates
...
def run_cmd(
    ...
    worker = VendorSearchWorker(client=client, config=cfg)
...
def explain_cmd(ctx: click.Context, query_type: str, params: str) -> None:
    from lina_vendors.templates import get_template
